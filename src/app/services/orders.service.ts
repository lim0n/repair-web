...
export class OrdersService {
  ...
  getOrdersList(withDeleted?: boolean): Observable<IOrder[]> {
    const url = new URL(`${this.apiPathPrefix}/orders`, environment.apiUrl);
  ...
  getOrderById(id: string): Observable<IOrder> {
    const url = new URL(`${this.apiPathPrefix}/orders/${id}`, environment.apiUrl);
  ...
  getOrdersByUserId(user_id: string): Observable<IOrder[]> {
    const url = new URL(`${this.apiPathPrefix}/orders/user/${user_id}`, environment.apiUrl);
    return this._api.get<IOrder[]>(String(url));
  }
  ...
  hardDeleteOrder(id: string): Observable<any> {
    const url = new URL(`${this.apiPathPrefix}/orders/hard/${id}`, environment.apiUrl);
    return this._api.delete<void>(String(url));
  }
