...
export class OrderDetailsService {
  ...
  getOrderDetailsList(withDeleted?: boolean): Observable<IOrderDetails[]> {
    const url = new URL(`${this.apiPathPrefix}/order-details`, environment.apiUrl);
  ...
  getOrderDetailsById(id: string): Observable<IOrderDetails> {
    const url = new URL(`${this.apiPathPrefix}/order-details/${id}`, environment.apiUrl);
    return this._api.get(String(url));
  }

  getOrderDetailsByOrderId(order_id: number): Observable<IOrderDetails[]> {
    const url = new URL(`${this.apiPathPrefix}/order-details/order/${order_id}`, environment.apiUrl);
  ...
  hardDeleteOrderDetails(id: string): Observable<any> {
    const url = new URL(`${this.apiPathPrefix}/order-details/hard/${id}`, environment.apiUrl);
    return this._api.delete<void>(String(url));
  }
